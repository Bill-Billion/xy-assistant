...
## 模糊感受多轮澄清方案（2025-11-05 15:01）

### 1. 多源环境感知管道
- 天气：保留本地天气缓存，模糊感受触发时自动注入室外温湿度。
- 家庭环境：预留 home_status 字段，未来接入温湿度、空调状态等。
- 用户画像：纳入慢病、用药、活动等信息，构造 last_vitals 占位。
- 行为日志：记录历史意图，供模型推测。

### 2. 大模型意图推理与澄清协议
- 全部交给豆包模型，无模板匹配；提示词要求在模糊场景提供 intent_hypotheses、clarify_question、suggested_actions、context_used。
- 置信度策略：≥0.8执行；0.5~0.8执行+澄清；<0.5纯澄清。

### 3. 多轮对话状态机
- ConversationState 新增 clarify_stage、clarify_history、last_hypotheses。
- 第一轮生成澄清问句；第二轮依据选项落地；最多两轮，超时给默认建议。
- 澄清阶段继续引导模型说明已执行内容并提示用户纠正。

### 4. 环境融合提示词示例
- 提示模型说明拥有天气/home_status/用户健康数据；模糊感受时从环境推断原因，生成候选意图与澄清。

### 5. 502 异常与稳定性
- 捕获天气/LLM 超时（10s），返回结构化错误；weather service 允许一次重试并记录日志。
- 健康建议兜底时说明缺乏环境数据。

### 6. 落地产出步骤
1. 更新系统提示词与 JSON schema；
2. 改造 IntentClassifier 支持 intent_hypotheses/clarify_question；
3. ConversationManager 支持多轮澄清状态；
4. 构造模拟 home_status 等环境数据用于测试；
5. 编写“我好热”等对话回归测试；
6. 监控澄清触发率与用户选择分布。
