...
| `app/services/weather_client.py` / `weather_service.py` | 阿里云天气 API + OSM 地理编码，提供 `weather_summary/weather_detail` 并写入 prompt/meta。 |
...
- **实时天气接入**：WeatherService 基于阿里云 GPS-to-weather + Nominatim 地理编码，结合 LLM 兜底解析地名/日期，缓存 `(location,date)`，并在 `function_analysis` 中填充 `weather_summary/weather_detail`、`weather_condition/weather_judgement/weather_evidence`。
